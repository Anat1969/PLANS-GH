...
// system prompt נפרד להפקת שיתופים/דוחות — כותב ניהולי/עירוני, ללא סירובים וללא JSON
const SHARE_SYSTEM_PROMPT = `אתה כותב מקצועי המשרת את אדריכלית העיר אשדוד בהפקת שיתופים ודוחות למקבלי החלטות (מנכ״ל העירייה, ראש העיר).
תפקידך לכתוב תוכן ניהולי/עירוני מוגמר, ברור ומשכנע, על בסיס הנתונים שסופקו בלבד.
כללים:
- כתוב בעברית בלבד, Markdown קצר (כותרות משנה ## ורשימות - כשמתאים).
- אל תמציא נתונים. אם חסר מידע מהותי — כתוב במפורש "מידע חסר להשלמת הדוח" ופרט מה נדרש להשלים.
- ללא הקדמות מתנצלות, ללא הערות מטא, ללא בקשות הבהרה — הפק את הסעיף המבוקש ישירות.
- החזר אך ורק את תוכן הסעיף עצמו (טקסט), ללא JSON וללא code-fences.
- הקפד על הסגנון והמיקוד שהוגדרו בהודעת המשתמש עבור קהל היעד.`;

// בחירת מודל לפי סוג הפעולה — Haiku זול/מהיר לרוב, Sonnet לניתוחים כבדים
const HEAVY_ACTIONS = new Set(['analyze_document', 'generate_opinion', 'pros_cons_analysis', 'generate_share']);
function pickModel(actionId) {
  return HEAVY_ACTIONS.has(actionId) ? 'claude-sonnet-5' : 'claude-haiku-4-5-20251001';
}
// פעולות שאינן משתמשות בפרסונת "מנוע הבדיקה" ואינן דורשות פלט JSON
const PLAIN_ACTIONS = new Set(['generate_share']);
...
function buildUserMessage(actionId, payload = {}) {
  ...
  const templates = {
    ...
    // הפקת סעיף שיתוף — הטקסט המלא של ההנחיה נבנה בצד הלקוח (buildShareSectionPrompt)
    generate_share: (p) => p.prompt || '[חסרה הנחיה]',
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const isPlain = PLAIN_ACTIONS.has(action_id);
    const systemPrompt = isPlain ? SHARE_SYSTEM_PROMPT : SYSTEM_PROMPT;
    ...
    // פעולות טקסט חופשי (הפקת שיתוף) — הטקסט הוא התשובה, ללא ניסיון פרסור JSON
    let parsed = null;
    if (isPlain) {
      parsed = { text: raw };
    } else {
